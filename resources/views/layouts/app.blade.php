        ...
        <aside class="w-64 bg-maroon-950 text-white flex flex-col shrink-0 fixed inset-y-0 left-0 z-30">
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                @php
                    $navItems = [
                        ...
                        ['route' => 'security.two-factor', 'label' => 'Keamanan 2FA', 'icon' => 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z'],
                    ];
                @endphp
                ...
                @foreach($navItems as $item)
                    @php
                        $isActive = match($item['route']) {
                            ...
                            'security.two-factor' => request()->routeIs('security.*'),
            ...
            <div class="px-4 py-4 border-t border-white/10 space-y-3">
                <div class="px-3 py-2 rounded-lg bg-white/5">
                    <p class="text-xs text-white/50">Login sebagai</p>
                    <p class="text-xs font-medium text-white/90 truncate">{{ auth()->user()->email }}</p>
                </div>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full text-left px-3 py-2 rounded-lg text-sm text-white/70 hover:bg-white/10 hover:text-white transition-colors">
                        Logout
                    </button>
                </form>
            </div>
